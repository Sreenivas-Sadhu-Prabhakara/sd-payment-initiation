...
| POST | `/v1/payment-instruction-procedure/initiate` | Initiate |
| GET | `/v1/payment-instruction-procedure` | Retrieve (list) |
| GET | `/v1/payment-instruction-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/payment-instruction-procedure/{crId}/update` | Update |
| PUT | `/v1/payment-instruction-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/payment-instruction-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-payment-initiation:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-payment-initiation ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-payment-initiation` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
